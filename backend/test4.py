...
class STT:
    ...
    def transcribe(self):
        ...
        while self.is_listening:
            ...
            segments, info = self.model.transcribe(
                audio_data, beam_size=5, language=self.language, vad_filter=True)
    ...
    def listen(self):
        ...
        self.recorder.listen_in_background(
            source=source, callback=self.recorder_callback)
    ...
    def setup_mic():
        ...
        try:
            ...
            logging.error(
                "Default input device not found. Printing all input devices:")
            for i in range(p.get_device_count()):
                info = p.get_device_info_by_index(i)
                if info['maxInputChannels'] > 0:
                    logging.info(
                        f"Device index: {i}, Device name: {info['name']}")
